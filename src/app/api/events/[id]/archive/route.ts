// Archive / restore / close — soft delete so nothing is permanently lost.
...
export async function POST(
  ...
  try {
    ...
    await recordAudit({
      ...
      text: `أرشفة حدث ${ev.name}`, entity: 'event', entityId: id,
      action: 'event_archive', ipAddress: getRequestIp(request),
    })
    return NextResponse.json({ message: 'تم أرشفة الحدث' })
  } catch (error) {
    return handleApiError(error, 'Archive error:')
  }
}
